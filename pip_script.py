...
def pip3install(packge=None, keyword='清华',cmd = 'pip'):
    ...
    if packge == None:
        ...
        option = cmd + ' install '+ source[keyword] + str(packge)

        print("option : \n "+ option)

        __import__('os').system(option)

if __name__ == '__main__':
    print("pelace in put pkg name:")
    packgename = input()
    change_range = {1:'阿里',2:'清华',3:'中科大',4:'华中理工大',5:'山东理工大',6:'豆瓣'}
    print('pelace change src:',change_range)
    changeNo = change_range[int(input())]
    pip3install(packge=packgename,keyword=changeNo)
